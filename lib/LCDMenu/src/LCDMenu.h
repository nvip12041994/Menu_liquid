...
// File header
    ...
    #if ARDUINO >= 160
        #define _LCDMenu_arduino_version         1    // for new Arduino version like 1.6.0 or higher
    #else
        #define _LCDMenu_arduino_version         0    // for old Arduino version like 1.0.6 or 1.0.5
    #endif
    ...
    // button flags
    #define _LCDML_REG_button_reserve_1                     7
    #define _LCDML_REG_button_reserve_2                     6
...
//# =======================
    class LCDMenu
    {
        private:
            // object pointer
            LCDMenu_menu *rootMenu;
            LCDMenu_menu *curMenu;
            LCDMenu_menu *activMenu;
            ...
            void    MENU_goMenu(LCDMenu_menu &p_m, uint8_t p_back);             // go to a menu element
            uint8_t MENU_countChilds(LCDMenu_menu *menu, uint8_t all=false);    // how many children exists on next layer
            ...
            boolean OTHER_searchFunction(LCDMenu_menu &p_m, uint8_t mode, LCDML_FuncPtr_pu8 p_search, uint8_t p_id);

        public:

            // constructor
            LCDMenu(LCDMenu_menu &p_r ,const uint8_t p_rows, const uint8_t p_cols, LCDML_FuncPtr contentUpdate, LCDML_FuncPtr contentClear, LCDML_FuncPtr menuControl);
            ...
            LCDMenu_menu * MENU_getDisplayedObj();                  // get the objection with the current conten to display                    
            LCDMenu_menu * MENU_getCurrentObj();                    // get the current child object
            ...
            void    BT_special(void);                                   // set button SPECIAL
            ...
            boolean BT_checkSpecial(void);                              // check special button
